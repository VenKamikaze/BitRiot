...
#define WINDOW_TITLE      "Bit Riot Final Beta Version"
		...
		case WM_MOVE:
			{
				...
				clientArea.left = clientX;
				clientArea.top = clientY;	
				clientArea.right = clientX + WINDOW_WIDTH -1 ;
				clientArea.bottom = clientY + WINDOW_HEIGHT - 1;
				return 0;
			} break;
...
int consoleInit()
{
	...
	// initialise direct draw 7 pointer
	if (FAILED(DirectDrawCreateEx(NULL, (void **)&lpdd, IID_IDirectDraw7, NULL)))
	{
		// failed to create DirectDraw 7 object, show error and quit
		MessageBox(main_window_handle, 
			"Failed to initialise IDirectDraw 7 Interface!", 
										...
										DDSCL_NORMAL)))
	{
		...
		MessageBox(main_window_handle, 
			"Failed to set cooperative level, check that your screen display meets the system requirements.", 
			NULL, MB_ICONEXCLAMATION);
		SendMessage(main_window_handle,WM_CLOSE,0,0);
		return(0);	
	}

	//clear ddsd and set size, flags
	...
	if (FAILED(lpdd->CreateSurface(&ddsd, &lpddsprimary, NULL)))
	{
		...
		MessageBox(main_window_handle, 
			"Failed to set display surface, check that your screen display meets the system requirements.", 
			NULL, MB_ICONEXCLAMATION);
		SendMessage(main_window_handle,WM_CLOSE,0,0);
		return(0);
	}
	...
	ddbltfxClear.dwFillColor = (255 << 16) + (100 << 8); // color for filling in back buffer

	// init rect for blitting to primary
	backRect.left = 0;
	backRect.top = 0;
	backRect.right = WINDOW_WIDTH - 1;
	backRect.bottom = WINDOW_HEIGHT - 1;
	...
	//DSound_Init();
	DSound * initDSound = DSound::getInstance();
	...
	erf->initSurfaces(lpdd, main_window_handle);

	engine = new GameEngine(lpdd, lpddsback);
